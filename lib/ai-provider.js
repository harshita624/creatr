...
export async function generateTextWithFallback({
  ...
  const providers = [
    ...
    },
    {
      name: "groq",
      enabled: Boolean(process.env.GROQ_API_KEY),
      run: () => askGroqText({ system, user, temperature, maxTokens }),
    },
    {
      name: "gemini",
      enabled: Boolean(process.env.GEMINI_API_KEY),
      run: () => askGeminiText({ system, user, temperature, maxTokens }),
    },
  ];
  ...
  const finalError = new Error(
    `All AI providers failed. ${
      errors.join(" | ") || "No provider is configured."
    }`
  );
...
async function askGroqText({ system, user, temperature, maxTokens }) {
  const groq = new Groq({
...
async function askGeminiText({ system, user, temperature, maxTokens }) {
  const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
